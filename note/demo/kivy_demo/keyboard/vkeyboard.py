'''
这个是kivy 官方虚拟键盘的源码， 加了注释和demo, kivy 的虚拟键盘设计的很奇妙， app 里允许村多个键盘，而且布局样式的可配置的
...
这个VKeyboard类是kivy内实现的触摸键盘，它对用是开发的可以定制，但是不建议直接实例它，建议先看怎么
请求（调出键盘）部分
...
the format::
    ...
    [ <text displayed on the keyboard> 展示字符, <text to put when the key is pressed 键值文本>,
      <text that represents the keycode> 内部键码, <size of cols> 宽度]
...
class VKeyboard(Scatter):
    ...
    """键盘布局种类加载目录属性(str) 默认值`data/keyboards/` """
    ...
    def _load_layout_fn(self, fn, name):
        ...
        try:
            ...
            logger.error(f"加载布局{name}异常:{err}", exc_info=True)
    ...
    def draw_keys(self):
        ...
        # -------- 分开绘制键和字体，以避免每次重新加载纹理 ---------

        # 绘制按键正常状态的背景图
        key_image = self.key_disabled_background_normal if self.disabled else self.key_background_normal
        key_normal = resource_find(key_image)
    ...
    def on_textinput(self, *largs):
        pass
        # on_key_down=lambda *args: Window.dispatch('on_key_down', args[0]),
        # on_key_up=lambda *args: Window.dispatch('on_key_up', args[0]),
        # on_textinput=lambda *args: Window.dispatch('on_textinput', args[0]))
