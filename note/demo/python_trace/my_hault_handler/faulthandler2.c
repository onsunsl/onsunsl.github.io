...
/* Call faulthandler2.enable() if the PYTHONFAULTHANDLER environment variable
   is defined, or if sys._xoptions has a 'faulthandler2' key. */

static int
faulthandler2_env_options(void)
{
    ...
    if (!((p = Py_GETENV("PYTHONFAULTHANDLER")) && *p != '\0')) {
        ...
        key = PyUnicode_FromString("faulthandler2");
        if (key == NULL)
            return -1;
    ...
    module = PyImport_ImportModule("faulthandler2");
    if (module == NULL) {
        return -1;
    }
...
int _PyFaulthandler2_Init(void)
{
#ifdef HAVE_SIGALTSTACK
    int err;
    ...
    if (!thread.cancel_event || !thread.running) {
        PyErr_SetString(PyExc_RuntimeError,
                        "could not allocate locks for faulthandler2");
        return -1;
    }
    PyThread_acquire_lock(thread.cancel_event, 1);
#endif

    return faulthandler2_env_options();
}

void _PyFaulthandler2_Fini(void)
{
#ifdef FAULTHANDLER_USER
    unsigned int signum;
#endif
